...
load(paste0(pathData,"data.RData"))
...
# Vemos que hay algunas numericas que estan mal catalogadas
dd[,16:20] <- lapply(dd[,16:20], as.numeric)
ListaVar <- sapply(dd,class)
...
# Canviamos de "character" a "factor"
...
lapply(names(VarCat),loopCat)
lapply(names(VarCat),loopCat1)
...
lapply(names(VarCat),loopbivarCat)
lapply(c(names(VarNum),names(VarInt)),loopbivarNum)
